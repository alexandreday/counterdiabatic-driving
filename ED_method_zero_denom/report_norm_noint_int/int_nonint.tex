...
\begin{figure}[!ht]
\begin{center}
\includegraphics[scale=0.72]{new_pics/slope_small_system_nonint.eps}
%\includegraphics[scale=0.7]{new_pics/v2_norm_mu_scaling_nonint.eps}
\caption{Using ED method, we obtain  $\mu$ dependence of negative of slope ($-\beta (\mu)$) is shown for non-integrable systems}
%\label{mu_scaling}
\end{center}
\end{figure}
...
Before we wind off this section, let's try $\mu_c^{(2)}(L) =\mu_0 L^2$ so as to see what happens if we parametrize with a wrong scaling. 
...
\section{transition and dissipation rate }
...
\end{equation}
We will need to use $G_{\lambda}(\mathcal{X} )= \partial_{\lambda} H + \dfrac{i}{\hbar} [\mathcal{X}, H] $ where $\mathcal{X}=A_{\lambda}(\mu) $ .

Now using Fermi's golden rule in \cite{kolodrubetz2016geometry},  transition rate $\langle \Gamma_n \rangle$  is derived using results from \cite{clerk2010introduction} is given by (details given in appendix \ref{sec.transition_rate}): 
\begin{align}
...
\end{equation}
 In small $\mu$ limit,  $\Gamma_n (\omega)$ is zero for all external energy $\omega$ through noise. In big $\mu$ limit, $ G_{\lambda} \approx \partial_{\lambda} H = \sum_i \sigma_i^x$. 

 \subsection{dissipation rate }
dissipation rate $\dfrac{d \sigma^2_E}{dt}$  is given by (details given in appendix \ref{sec.dissipation_rate}): 

\begin{align}
 \dfrac{d \sigma^2_E}{dt}&= \int d\omega \omega^2 \Gamma_n (\omega)\\
 &=-\kappa \langle n|[G_{\lambda}, H]^2 |n \rangle
\end{align}
In small $\mu$ limit,  $ \dfrac{d \sigma^2_E}{dt}$ is zero. In big $\mu$ limit, since, $ G_{\lambda} \approx \partial_{\lambda} H $.  Hence $[G_{\lambda}, H]^2= (C^{(1)}) ^2$ and $\dfrac{d \sigma^2_E}{dt}=-\kappa \langle n|(C^{(1)})^2 |n \rangle $
