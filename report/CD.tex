...
C^{(5)} &= \alpha^4  C^{(1)}  +C^{(2)}_{T_2}  + C^{(2)}_{T_3}
\end{align*}
...
Now we will prove some results, which we are useful in computations of commutators.
\begin{thm}
If $C^{(3)}=  \alpha^2  C^{(1)} + T$ , then $C^{(2n+1)}= \alpha^{(2n)} C^{(1)} + C^{(2n-2)}_T$, $\forall n>0$,  where $T$ is an operator,  $ C^{(0)}_T=T$ , $C^{(2)}_T=[H, [H, T]]$ and so on.
\end{thm}
