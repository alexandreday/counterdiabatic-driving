...
In thermodynamic limit, we see that exact gauge potential $A_{\lambda}$ has  $\mu=0$ and it has infinite norm. However, in thermodynamic limit, if we take $\mu \neq 0$, then we have approximate gauge potential. If we take $\mu \rightarrow \infty$ limit, then norm of approximate gauge potential is zero.
...
Now using Fermi's golden rule in \cite{kolodrubetz2016geometry},  transition rate $\Gamma_n$ is derived as: 
\begin{equation}
\Gamma_n= \kappa \sum_n \langle n | G^2_{\lambda}| n \rangle - \kappa  \sum_n \langle n |  \partial_{\lambda} H |    n \rangle ^2
\end{equation}
where only first term depends upon $\mu$.
...
\subsection{Formula involving commutators}
...
\subsection{ETH for complex systems}
...
We have a set of three couple linear recurrence difference equations:
\begin{equation}
\boxed{
\begin{aligned}
