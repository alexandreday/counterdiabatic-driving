...
\begin{equation}
i \hbar \partial_{\lambda} H= [ A_{\lambda}, H] -i \hbar M_{\lambda} | n (\lambda) \rangle \langle n (\lambda) |
\label{m_lambda}
\end{equation}
where 
\begin{equation}
M_{\lambda} = - \sum_n \dfrac{\partial E_n (\lambda)}{\partial \lambda}
\end{equation}
...
Any $A_{\lambda}$ satis   fying equation \ref{def_commutator} is an exact gauge potential. We note that  if $A_{\lambda}$ satisfies equation \ref{def_commutator} , then  $A_{\lambda}+ f(H)$, where $f(H)$ is any function that only contains terms involving Hamiltonian $H$ and other operators that commutes with $H$.

For our future purposes, let's study a gauge choice where we assume diagonal elements of $A_{\lambda}$ is zero. This should mean $\Tr(f(H)^2)=0$ and therefore, this is a choice with minimum Forbenius norm of  $A_{\lambda}$ as shown below:
\begin{align*}
||A_{\lambda}^{\prime} ||^2 = \Tr ( (A_{\lambda} + f(H) )^2) = \Tr (A_{\lambda}^2)  + \Tr(f(H)^2) + 2  \Tr ((A_{\lambda} f(H))=  \Tr (A_{\lambda}^2)
\end{align*}
...
We would be using  Hadamard (or sometimes called Baker-Hausdorff-Campbell) formula to simplify $\partial_{\lambda}H (t)$.
...
\end{eqnarray}
where $C^{(n)}$ is n- commutator of $H$ and $\partial_{\lambda} H$, i.e. $C^{(n)}= [H, [H, \mbox{ n times} \ldots,[H, \partial_{\lambda} H ]]] ] $. Properties of $C^{(n)}$ are noted in appendix \ref{sec.Cn}.
...
\end{align*}
Hence, now we can use analytical continuation to claim that our result is also true when $\alpha^2 / \mu^2 >1$ since there is no divergence when $\alpha^2 / \mu^2 =1$. Hence, both methods give the same answer as it should.
...
Now for future purposes, let's rotate our axes such that interchange $z$ and $x$ axes while keeping the y axis the same. Our Hamiltonian becomes:
...
We would study another integrable model, which is called Transverse Field Ising model. This model shows quantum phase transition between ferromagnetic and paramagnetic phases. This model satisfies Ising symmetry $G= \Pi_i \sigma_i^z$ since $[H, G]=0$, where $H$ is the Hamiltonian. It's Hamiltonian is given by:
\begin{equation}
...
This model can be written in terms of spinless fermions ($c_i, c^{\dagger}_i $) using Jordan- Wigner transformation.
...
\begin{equation}
A_{\lambda}= \sum_l \alpha_l O_l
\end{equation}
where $O_l$ is given by
\begin{equation}
O_l= 2 i \sum_j (c^{\dagger}_{j} c^{\dagger}_{j+l} - \mbox{h.c})
\end{equation}

It will be good to find either exact or approximate gauge potential using our regulator method.
