...
\begin{figure}[!ht]
\begin{center}
\includegraphics[scale=0.7]{new_v1_norm_L8_L10_L12.eps} 
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[scale=0.7]{slope.eps}
...
\caption{Two different scaling regimes of $1/\mu$ and $1/\mu^4$: $y_1=\alpha_1\mu^{\beta_1}$ and $y_2=\alpha_2\mu^{\beta_2}$ where $\beta_1=-1.02$, $\beta_2=-3.88$, $\alpha_1=0.682$ and $\alpha_2=328.38$ which are found using least square method}
...
\begin{itemize}
\item \textbf{Constant in $\mu$ regime} when $\mu \ll \min\{w_{nm}\}$: In this regime, $\mu$ is so small that it doesn't really affect the norm of gauge potential. $\min\{w_{nm}\}= \Delta= E_1 - E_0$ So, we get exact gauge potential in this regime.
\begin{equation}
...
\item \textbf{$1/\mu^4$ scaling regime} when $\mu \gg \max\{w_{nm}\}$:
$\max\{w_{nm}\}= E_{HES} - E_{GS}$
\begin{align*}
||A_{\lambda}||^2 &= \dfrac{1}{ \mu^4} \sum_n \sum_{m \neq n}  \omega_{nm}^2 |\langle m | \partial_{\lambda}H| n \rangle|^2 = \dfrac{1}{ \mu^4} \Tr  [H, \partial_{\lambda}H]^2 =\dfrac{\alpha_2^{Th}}{ \mu^4} \sim\dfrac{L}{ \mu^4} 2^L 
\end{align*}
where theoretical value of  $\alpha_2^{Th}=\Tr  [H, \partial_{\lambda}H]^2 $ should be compared against the numerical value obtained in figure \ref{nonintegrable_mu}.  For $L=12$, we obtain $\alpha_2^{Th}=119.41$ whose details are given in appendix B.
...
\item \textbf{Intermediate regime of $1/\mu$ scaling:} For non-integrable systems, we can use ETH to claim that
\begin{equation}
\dfrac{||A_{\lambda}||^2}{2^L}= \dfrac{\pi }{4 \mu} \langle |f_O(E_n)|^2  \rangle \sim \dfrac{L^\gamma }{ \mu} 2^L
\end{equation}
where $\gamma$ is unknown.
\end{itemize}
...
\item \textbf{Checks on my numerical results:}
\begin{itemize}
...
\item Numerically, go for bigger $\mu$ so that $\alpha_2^{Th}$ matches exactly with numerical value of $\alpha_2$
\end{itemize} 
