...
H= J \sum_{j} \sigma_j^x(\sigma_{j+1}^x+ \sigma_{j-1}^x) + \lambda \sum_{j} \sigma_j^z 
\end{equation}
where we have chosen $J=1$ and $\lambda=5$ (and maybe also $\lambda=-5$?) with open boundary conditions.
...
Since anti-ferromagnetic phase has more local order compared to ferromagnetic phase, we expect the former to be less affected by finite size effects. We should choose parameters such that we are in ferromagnetic phase.
\appendix
