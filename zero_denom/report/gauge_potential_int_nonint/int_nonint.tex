...
\begin{equation}
H= -J \sum_{j=1}^{L-1} \sigma_j^x \sigma_{j+1}^x - \lambda \sum_{j} \sigma_j^z 
\label{xx_z}
\end{equation}
where we have chosen open boundary conditions and $\lambda$ is externally-controlled transverse magnetic field.
...
\begin{align*}
O_{l=1}&=  \sum_{j=1}^{L-1} ( \sigma_j^x  \sigma_{j+1}^y +  \sigma_j^y  \sigma_{j+1}^x) \\
O_{l=2} &=  \sum_{j=1}^{L-2} ( \sigma_j^x \sigma_{j+1}^z \sigma_{j+2}^y +  \sigma_j^y \sigma_{j+1}^z \sigma_{j+2}^x) 
\end{align*}

On computation, we find that with open boundary conditions, we get $\Tr O_l  O_p= \delta_{l,p} 2^{L+1} (L-l)$
...
\includegraphics[scale=0.7]{new_pics/int_comparing_num_anal_norm_L_scaling.eps}
...
&=  2^{L+1} \sum_{l=1}^{L-1}  (L-l) \alpha_l ^2
\end{align}
...
\caption{Integrable systems: exact gauge potential as a function of system size }
\end{center}
\end{figure}
